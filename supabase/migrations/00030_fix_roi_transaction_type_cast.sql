-- Ensure 'roi_credit' exists in the transaction_type enum and cast it correctly in the function
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_type t JOIN pg_enum e ON t.oid = e.enumtypid WHERE t.typname = 'transaction_type' AND e.enumlabel = 'roi_credit') THEN
        ALTER TYPE transaction_type ADD VALUE 'roi_credit';
    END IF;
EXCEPTION
    WHEN duplicate_object THEN NULL;
END $$;

-- Update the function to cast explicitly
...
BEGIN
  ...
  LOOP
    ...
    IF roi_amount > 0 THEN
      ...
      -- Log transaction with explicit cast
