CREATE OR REPLACE FUNCTION public.get_referral_chain(user_id uuid)
 RETURNS TABLE(referrer_id uuid, level integer, commission_rate numeric)
 LANGUAGE plpgsql
AS $function$
declare
...
begin
    ...
    while current_referrer is not null and current_level <= 15 loop
        -- Get overrides from referrer's profile
        select referral_levels_overrides into v_overrides from public.profiles where id = current_referrer;
        
        -- Try to get override for current level
        if v_overrides is not null and v_overrides ? ('level' || current_level || '_commission') then
            v_rate := (v_overrides->>('level' || current_level || '_commission'))::numeric / 100.0;
        else
