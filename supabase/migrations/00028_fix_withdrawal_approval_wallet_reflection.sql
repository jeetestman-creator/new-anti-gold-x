-- Fix process_withdrawal_approval to correctly reflect in withdrawal wallet
...
BEGIN
  ...
  ELSE
    ...
    UPDATE wallets SET 
      balance = balance + withdrawal_rec.amount,
      updated_at = NOW()
    WHERE user_id = withdrawal_rec.user_id AND wallet_type = v_wallet_type::WALLET_TYPE;
